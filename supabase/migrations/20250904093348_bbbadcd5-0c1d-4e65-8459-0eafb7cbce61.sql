-- Update send_notification function to trigger email notifications
CREATE OR REPLACE FUNCTION public.send_notification(user_id_param uuid, title_param text, message_param text, type_param text, data_param jsonb DEFAULT NULL::jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
    ...
    PERFORM
      net.http_post(
        ...
        headers := jsonb_build_object(
          ...
          'Authorization', 'Bearer ' || current_setting('request.jwt.claims', true)::jsonb->>'sub'
        ),
...
-- Ensure all booking status change triggers are properly configured for emails
-- Update the booking status change handler to include proper email data
...
BEGIN
  ...
  IF NEW.status != OLD.status THEN
    ...
    IF faculty_user_id IS NOT NULL THEN
      ...
      ELSE
        email_subject := format('Hall Booking Update - %s - %s', hall_name, UPPER(REPLACE(NEW.status, '_', ' ')));
...
-- Update the new booking handler to include proper email notifications
